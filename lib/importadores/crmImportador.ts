  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        console.log("CRM JSON:", json);

        const registros = json.map((row: any) => {
          const lista = String(row["LISTA"] || "")
            .trim()
            .toUpperCase();

          let etapa = "CONTATO";

          if (lista.includes("AULA EXPERIMENTAL AGENDADA")) {
            etapa = "AGENDADO";
          } else if (lista.includes("RETORNO FECHAMENTO")) {
            etapa = "AULA_EXPERIMENTAL";
          } else if (lista.includes("VENDA FECHADA")) {
            etapa = "FECHADO";
          }
